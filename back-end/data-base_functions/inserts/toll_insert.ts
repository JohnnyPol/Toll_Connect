		...
		for (const station of tollStations) {
			try {
				...
				if (station.PM !== 'ΜΤ' && station.PM !== 'ΠΛ') {
					throw new Error(
						`Invalid PM value ${station.PM} for station ${station.Name}. Must be either 'ΜΤ' or 'ΠΛ'`,
					);
				}
				...
				console.log(
					`Successfully inserted toll station: ${station.Name}`,
				);
			} catch (err) {
				console.error(`ERR insert toll station ${station.Name}:`, err);
				throw err;
			}
		}
